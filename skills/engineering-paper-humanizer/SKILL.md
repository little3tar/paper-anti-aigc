...
无论使用哪种入口，humanizer 的默认完成状态都是”降低 AI 检测风险”。改写时优先改变句式骨架、语序和连接方式，降低过度成熟、过度整齐、过度精炼的模型痕迹；可把非关键专业表达改成更普通的说法，允许适度口语化。默认保留并适当增加”的、了、把、这个、这样、前面算出来的、后面还需要”等普通承接词。数值、单位、型号、标准号、引用和核心技术关系仍需保留。
