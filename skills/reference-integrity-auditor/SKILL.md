...
5a. **检查 source marker 完整性**
   ...
   - 残留标记（`[待补来源: ...]`、`[标准规范: ...]`、`[用户材料: ...]`、`[Mxx]`）的完整定义和清理规则见 chapter-writer 参考文件 `citation-key-rules.md` §禁止进入最终输出的工作标记。本阶段职责：将残留标记内容移入相应缺口清单或台账，`[用户材料: ...]` / `[Mxx]` 标记为 P3（最终由 format-cleaner 清除）。
...
7. **向用户报告所有问题并询问处理方式**
   ...
   - P2/P3 不阻塞下游，但**必须逐项列出**。Review-gated 模式下逐一询问用户处理方式：修复、延后、还是跳过；preauthorized 模式下记录默认处理决策（P2 修复、P3 延后至 format-cleaner）到 `audit.md`，可连续执行不逐一询问。用户对每个问题的决定记录到 `audit.md`。
   - 标准或规范来源缺失时，检索方向必须包含标准名称、标准号候选、发布机构、年份/版本和条文范围。
