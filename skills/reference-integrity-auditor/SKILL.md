...
| "这个 claim 缺来源，Zotero 没找到，标记 P1 就行" | 缺来源时必须按三级检索协议依次尝试补全（见 source-policy.md），只有三级均未获取的才能标记为 P1 并列入证据缺口 |
...
5b. **对照文献笔记缓存校验正文 claim（⭐）**
   ...
   - 如某条待校验 claim 对应的文献笔记不在缓存中且状态非 `无笔记`/`获取失败`，可尝试通过 Zotero MCP 补获取（延续写作阶段的用户授权）。补获取同样每篇最多 1 次，失败后标记 `获取失败` 并跳过。**补获取成功后必须立即将摘要写入 `literature-notes.md` 缓存并更新状态表**，不得仅口头确认内容后跳过落盘。
...
缓存的使用和读取规则见 `literature-notes-cache-rules.md`。缓存按章存储（`chapters/chX/literature-notes.md`）。
