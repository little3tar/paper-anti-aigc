...
遵循 workflow §来源策略中的强制三级检索协议：**必须依次尝试 Zotero → 用户 → 网络，每项都要尝试**，不得只查一项就放弃。只有三级都尝试过仍未获取的信息才能列入证据缺口。

1. Zotero MCP 或本地文献 MCP（第一级）。
2. 用户提供的文件、导出的 BibTeX/RIS/CSV、笔记、任务书和导师意见（第二级）。
3. 官方网站、标准数据库、期刊页面、专利、高校文库和厂商页面（第三级）。

标准规范不是普通文献。如果课题依赖行业标准、国家标准、规范、规程、验收要求或限值，即使用户的 Zotero 文库中没有匹配条目也应按三级协议检索：Zotero → 询问用户 → 搜索官方标准平台。已核验的标准应规划为正式论文证据，而不仅仅是背景说明。
