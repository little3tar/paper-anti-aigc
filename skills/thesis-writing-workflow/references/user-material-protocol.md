...
- 用户未回复此项 → 标记为 `[分类待确认]`（写入 `materials-inventory.md` 的数据性质列），写作前再次询问，不可默认写成 A 类产品定论
...
**文件命名规范**：用户提供的文件如为无信息量名称（如 `IMG_0001.jpg`、`微信图片_20240101.jpg`、`video_001.mp4`、`document.pdf`、`data.xlsx`），必须重命名为描述性名称后再整理（如 `截割臂装配图.jpg`、`摆角缸测试视频.mp4`、`YX3电机数据表.pdf`）。重命名前告知用户原名→新名映射，用户确认后执行。原始名称保留在编目表的备注中。
